...
const MetadataViewer = (function() {
  ...
  const IMAGE_KEYS = ['image', 'icon', 'logo', 'thumbnail'];
  ...
  function renderImagePreview(url) {
    ...
    return `
      <div class="meta-image-preview">
        <img src="${escapeHtml(url)}" alt="Preview" loading="lazy" />
      </div>
    `;
  }
  ...
  function renderMetadataSection(title, data, icon) {
    ...
    const rows = entries.map(([key, value]) => {
      ...
      const imagePreview = !Array.isArray(value) && isImageKey(key)
        ? renderImagePreview(value.toString())
        : '';

      return `
        <div class="meta-row${imagePreview ? ' has-preview' : ''}" data-full-value="${escapeHtml(fullValue)}">
  ...
  function show() {
    ...
    shadow.innerHTML = `
      <style>
        ...
        * {
        ...
        /* Image preview */
        .meta-image-preview {
          margin: 4px 8px 8px calc(90px + 16px);
          padding: 8px;
        ...
        .meta-image-preview img {
          ...
          object-fit: contain;
          border-radius: 4px;
          background: repeating-conic-gradient(#27272a 0% 25%, #18181b 0% 50%) 50% / 16px 16px;
        }

        .meta-image-preview img[src=""],
        .meta-image-preview img:not([src]) {
          display: none;
        }

        .meta-row.has-preview {
          margin-bottom: 0;
        }
    ...
    shadow.querySelectorAll('.meta-image-preview img').forEach(img => {
